...
menuFormConfig.formDataRaws = JSON.parse(JSON.stringify(formDataRaws));
...
/* ModalGenerator的配置项 */
const menuModalConfig: IModalConfig = {
  formItems: [
    {
      type: 'input',
      label: '菜单名',
...
formDataRaws = {};
menuModalConfig.formItems?.forEach(item => {
  formDataRaws[item.field] = item.type === 'datepicker' ? ['', ''] : '';
});
menuModalConfig.formDataRaws = JSON.parse(JSON.stringify(formDataRaws));
